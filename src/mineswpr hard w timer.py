...
def main():
    ...
    mineList = [[0] * 30 for i in range(16)]
    buttonList = []
    gameOver = 0
    find = 99
    gameStarted = 0
...
def around(x, y):
    for i in range(x - 1, x + 2):
        for j in range(y - 1, y + 2):
            if i >= 0 and i <= 15 and j >= 0 and j <= 29:
                if not (i == x and j == y):
...
def over():
    ...
    for i in range(16):
        for j in range(30):
            if mineList[i][j] == -1:
                buttonList[i][j].b.config(text="#", bg="red", fg="white")
    label["text"] = "Oops!"
    resetbt["text"] = "New Game"
    return
